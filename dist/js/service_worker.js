...
DownloadsInterceptManager.prototype.onDeterminingFilename = function (downloadItem, suggest) {
    ...
    browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
        if (tabs[0]) {
            // Get the title and sanitize it for use as filename
            ...
            // Combine title with extension
            let newFilename = title + extension;

            // Suggest the new filename
            suggest({ filename: newFilename });
        } else {
            // Fallback to original filename if we can't get the title
